...
/*
 * The naive solution would be to iterate through the possible binary strings and echck through the string (s) to see
...
bool Solution::hasAllCodesWithBitSet(std::string s, int k){
    int len = s.size(), count = 1 << k;
    if(k>len) return false;
    int num = k > 1 ? stoi(s.substr(len - k + 1), 0, 2) << 1 : 0;
